...
const MyCodeBlock = ({ language }: MyCodeBlockProps) => {
  //
  ...
  const [mounted, setMounted] = useState(false);
  const [mounted, setMounted] = useState(false);
  const [mounted, ssetMounted] = useState(false);
  `;
  ...
  useEffect(() => {
    ...
    if (!mounted) {
      return (
        <div role="status" className="my-3 mx-auto  w-fit">
          <svg
            aria-hidden="true"
  ...
  return (
    <div className="mt-2 max-h-64 overflow-y-auto code-block-scrollbar">
      <CopyBlock
        ...
        text={code}
        wrapLongLines
        theme={theme === "dark" ? nord : atomOneLight}
      />
    </div>
  );
};
